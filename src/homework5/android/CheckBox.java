...
public class CheckBox extends SimpleGroup implements Selectable {
	...
	public CheckBox(int x, int y, int width, int height, String text, Panel panel, Context context) {
	...
	public CheckBox(int x, int y, int width, int height, GraphicalObject display,Panel panel, Context context) {
		super(x,y,width,height);
		outer = new OutlineRect(3,3,25,28,Color.DKGRAY,2);
		inner = new OutlineRect(5,5,23,25,Color.LTGRAY,2);
	...
	public void setDisplay(GraphicalObject display) {
		this.display = display;
	}
	...
	public void onClick() {
		if (selected) {
			Toast.makeText(context, "CheckBox " + label.getText() + " changed check!", Toast.LENGTH_SHORT).show();
			value = true;
		} 
	}
	@Override
